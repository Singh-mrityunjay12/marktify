...
class ProductController extends Controller
{
    ...
    public function storeProductStep2(Request $request)
    {
        ...
        //     $rules['preview_imgs.*'] = ['mimes:jpeg,jpg,png,gif,svg','max:1024'];
        // }
        ...
        */
        ...
        {   
            $arr3['file_id'] = (!empty($request->file_id[$key2])) ? $request->file_id[$key2] : unique_key();
        ...
        foreach ($finalArr as $key => $value)
        {
            ...
            // $obj = ProductMeta::firstOrNew(['product_id'=>@$request->product_id]);
     ...
     * Display the specified resource.
     */
    public function attach_files_view(string $id)
    {
        $data['data'] = Product::find($id);
        $data['productMeta']= (object) ProductMeta::where('product_id',$id)->pluck('value','key')->toArray();
    ...
    {
        $newfilename = (!empty($request['file_name']) ? $request['file_name'] : '').'.'.substr($request['file_url'], strrpos($request['file_url'], '.') + 1);

        if(isset($request['file_url']) && !empty($request['file_url'])){
            if(Storage::exists($request['file_url'])){
                return Storage::download($request['file_url'],$newfilename);
            }
            return;
        }
    ...
    public function update_product_review_status(Request $request){
        ...
        if($request->status == 1 && empty($obj->published_at))
        $obj->published_at = now();
        ...
        $obj->save();
        return response()->json(['status' => true,'msg' =>trans('msg.status_update'),'url'=>route('admin.product.index')], 200);
    }
    ...
    public function storeMedia(Request $request)
    {
        try {
            ...
            $filename = request()->file('file')->storeAs('tmp/uploads', $imageName);
           
            return response()->json([
                'name'          => $imageName,
                'original_name' => $file->getClientOriginalName(),
                'msg'  => trans('msg.file_upload'),
                'status' => true,
            ],200);
        } catch (\Throwable $th) {
            return response()->json(['status' => false,'msg'  => trans('msg.something_went_wrong')],500);
        }
    }
}
